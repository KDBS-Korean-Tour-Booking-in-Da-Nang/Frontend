...
const VoucherCreateModal = ({ isOpen, onClose, onSuccess, tours, companyId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        status: 'ACTIVE',
        tourIds: form.tourIds && form.tourIds.length > 0 ? form.tourIds : null
      };
  ...
  // Calculate dropdown panel max height based on first item height (show max 7 items)
  useLayoutEffect(() => {
    if (tourDropdownOpen && firstItemRef.current) {
      const itemHeight = firstItemRef.current.offsetHeight || 40;
      setPanelMaxH(itemHeight * 7 + 8);
    }
